...
describe("evaluateAudit — severity blocking", () => {
	it("blocks on high severity", () => {
		const r = evaluateAudit(
			[advisory({ severity: "high", name: "lodash", ghsa: "GHSA-r5fr-rjxr-66jc" })],
			ledgerEmpty,
			today,
		);
...
describe("evaluateAudit — moderate CWE blocking", () => {
	it("blocks moderate with CWE-79 (XSS)", () => {
		const r = evaluateAudit(
			[advisory({ severity: "moderate", cwe: ["CWE-79"] })],
			ledgerEmpty,
			today,
		);
		expect(r.exitCode).toBe(1);
	...
	it("blocks moderate with CWE-1321 (proto pollution)", () => {
		const r = evaluateAudit(
			[advisory({ severity: "moderate", cwe: ["CWE-1321"] })],
			ledgerEmpty,
			today,
		);
		expect(r.exitCode).toBe(1);
	...
	it("blocks moderate with CWE-918 (SSRF)", () => {
		const r = evaluateAudit(
			[advisory({ severity: "moderate", cwe: ["CWE-918"] })],
			ledgerEmpty,
			today,
		);
		expect(r.exitCode).toBe(1);
	...
	it("blocks moderate with CWE-287 or CWE-863 (auth)", () => {
		const r1 = evaluateAudit(
			[advisory({ severity: "moderate", cwe: ["CWE-287"] })],
			ledgerEmpty,
			today,
		);
		const r2 = evaluateAudit(
			[advisory({ severity: "moderate", cwe: ["CWE-863"] })],
			ledgerEmpty,
			today,
		);
	...
	it("blocks moderate without ledger entry even when CWE is non-blocking", () => {
		const r = evaluateAudit(
			[advisory({ severity: "moderate", cwe: ["CWE-346"] })],
			ledgerEmpty,
			today,
		);
		expect(r.exitCode).toBe(1);
...
describe("evaluateAudit — clean run", () => {
	...
	it("does not gate low or info severities", () => {
		const r = evaluateAudit(
			[advisory({ severity: "low", cwe: ["CWE-79"] }), advisory({ severity: "info" })],
			ledgerEmpty,
			today,
		);
		expect(r.exitCode).toBe(0);
