...
const LEDGER_REQUIRED_FIELDS = [
  'ghsa', 'package', 'via', 'severity', 'cwe', 'category',
  'rationale', 'mitigation', 'accepted_by', 'accepted_on',
  'expires', 'review_action',
];
...
function validateLedgerEntry(entry, idx) {
  ...
  if (entry.ghsa && !GHSA_RE.test(entry.ghsa)) errs.push(`entry[${idx}]: ghsa '${entry.ghsa}' does not match GHSA pattern`);
...
export function evaluateAudit(advisories, ledger, todayISO) {
  ...
  for (const e of ledger) {
    if (e.ghsa && !liveGhsas.has(e.ghsa)) {
      warnings.push(`ledger entry refers to ${e.ghsa} (${e.package}) which is no longer flagged by npm audit — consider removing`);
    }
  }
  ...
  for (const adv of advisories) {
    ...
    if (ledgerEntry) {
      ...
      warnings.push(`moderate advisory ${adv.ghsa} (${adv.name}) allowlisted via ledger; expires ${ledgerEntry.expires}`);
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    if (parsed == null) {
      ...
      console.error(`audit-gate: ledger YAML root must be a list (got ${typeof parsed} '${Array.isArray(parsed) ? 'array' : Object.prototype.toString.call(parsed)}') — see frontend/security/README.md for the schema`);
      process.exit(1);
    }
  } catch (e) {
    if (e.code !== 'ENOENT') {
