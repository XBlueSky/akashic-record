<script lang="ts">
	...
	function timeAgo(dateStr: string | null): string {
		...
		return (
			$t("repo.daysAgo", { values: { days: Math.floor(hours / 24) } }) ||
...
<div class="flex h-full w-full flex-col overflow-hidden">
	...
	<div class="shrink-0 border-b border-border/50 bg-card/80 backdrop-blur-xl px-4 py-3">
		...
		<div class="flex items-center gap-3 min-w-0">
			...
			{#if detail?.source_type === "website"}
				<span
					class="font-mono text-[10px] font-semibold tracking-widest border border-border rounded px-2 py-0.5 text-muted-foreground shrink-0"
				>
		...
		{#if detail}
			<div
				class="mt-2 flex items-center gap-2 font-mono text-xs tracking-wider text-slate-400 flex-wrap"
			>
				{#if detail.source_type === "website"}
					<span class="inline-flex items-center gap-1">
						<FileText size={12} strokeWidth={1.5} />
						{detail.total_documents}
						{$t("repo.documents") || "documents"}
					</span>
					<span class="h-[3px] w-[3px] rounded-full bg-muted-foreground/40"></span>
					<span class="inline-flex items-center gap-1">
						<List size={12} strokeWidth={1.5} />
						{detail.total_sections}
						{$t("repo.sections") || "sections"}
					</span>
				{:else}
					<span class="inline-flex items-center gap-1">
						<Database size={12} strokeWidth={1.5} />
						{$t("repo.chunks", { values: { count: detail.total_chunks } }) ||
							`${detail.total_chunks} chunks`}
					</span>
					<span class="h-[3px] w-[3px] rounded-full bg-muted-foreground/40"></span>
					<span class="inline-flex items-center gap-1">
						<FolderTree size={12} strokeWidth={1.5} />
						{$t("repo.modules", { values: { count: detail.total_modules } }) ||
							`${detail.total_modules} modules`}
					</span>
				{/if}
				{#if detail.latest_job}
					...
					<span
						>{$t("repo.lastIndexed", {
							values: {
								time: timeAgo(detail.latest_job.completed_at || detail.latest_job.started_at),
							},
						}) ||
							`indexed ${timeAgo(detail.latest_job.completed_at || detail.latest_job.started_at)}`}</span
					>
				{/if}
			</div>
		{:else if loading}
				...
				<div class="flex items-center justify-between font-mono text-xs">
					...
					{#if activeJob.total_files > 0}
						<span class="text-muted-foreground">
							{activeJob.processed_files}/{activeJob.total_files} files ({progressPercent(
								activeJob,
							)}%)
						</span>
					{/if}
		...
		<nav class="mt-3 flex items-center gap-1" aria-label="Repository views">
			{#each [{ id: "timeline", label: $t("repo.timeline") || "Timeline" }, { id: "graph", label: $t("repo.graphTab") || "Graph" }, { id: "modules", label: $t("repo.modulesTab") || "Modules" }] as tab (tab.id)}
				<a
					href={tabHref(tab.id as "timeline" | "graph" | "modules")}
					class="rounded-md px-3 py-1 font-mono text-xs font-semibold tracking-wide transition-all {isTabActive(
						tab.id,
					)
			...
			<AlertDialog.Description class="text-slate-400">
				{#if detail}
					{$t("repo.purgeDesc", {
						values: {
							name: detail.name,
							chunks: detail.total_chunks,
							modules: detail.total_modules,
						},
					}) ||
						`Permanently delete "${detail.name}"? This will remove ${detail.total_chunks} chunks across ${detail.total_modules} modules.`}
				{:else}
