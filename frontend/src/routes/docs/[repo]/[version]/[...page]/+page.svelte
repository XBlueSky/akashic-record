	...
	<div class="py-16 text-center" data-testid="docs-404">
		...
		<div class="mt-6 flex justify-center gap-3">
			<button
				...
				onclick={() => commandPalette.show()}>{$t("docs.notFound.search")}</button
			>
			<a
				href={base}
				class="rounded border border-border px-3 py-1.5 text-[13px] text-primary hover:underline"
			>
		...
		<article class="min-w-0 flex-1">
			...
			{#if rendered.toc.length > 0}
				<details class="mb-4 rounded border border-border px-3 py-2 xl:hidden">
					<summary class="cursor-pointer text-[11px] uppercase tracking-wider text-muted-foreground"
						>{$t("docs.toc")}</summary
					>
					<ul class="mt-2 space-y-1">
						{#each rendered.toc as e (e.id)}
							<li class={e.level === 3 ? "pl-4" : ""}>
								<a href={`#${e.id}`} class="text-[12px] text-muted-foreground hover:text-foreground"
									>{e.text}</a
								>
							</li>
						{/each}
