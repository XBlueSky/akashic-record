...
export function buildCallGraphLayout(
  ...
  const ghosts: MicroNode[] = external.map(g => ({
    id: g.id, name: g.name, chunk_type: g.chunk_type, line_count: 0,
    x: cx + (Math.random() - 0.5) * 120,
    y: cy + (Math.random() - 0.5) * 120,
    isGhost: true, ghostModuleId: g.module_id,
    ghostModulePath: g.module_path, direction: g.direction,
...
export function sectionEdgePath(
