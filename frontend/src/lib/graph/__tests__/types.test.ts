...
import {
  isClassType, isEnumType, getColorBucket, getChunkColors, getSectionColors,
  microNodeLabel, pillWidth, ASTRAL_COLORS, SECTION_DEPTH_COLORS,
} from '../types';
import type { MicroNode } from '../types';

const node = (over: Partial<MicroNode> = {}): MicroNode => ({
  id: 'n', name: 'thing', chunk_type: 'function', line_count: 1,
  x: 0, y: 0, inDegree: 0, outDegree: 0, ...over,
});
...
describe('type helpers', () => {
  ...
  it('getSectionColors clamps depth to palette length', () => {
    ...
    expect(getSectionColors(99, false)).toEqual(SECTION_DEPTH_COLORS[SECTION_DEPTH_COLORS.length - 1]);
  });
