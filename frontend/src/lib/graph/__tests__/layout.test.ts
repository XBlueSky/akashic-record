...
import { buildCallGraphLayout, buildSectionTreeLayout, edgeBezierPath, sectionEdgePath } from '../layout';
...
const chunk = (id: string, over: Partial<ChunkNode> = {}): ChunkNode => ({
  id, name: id, chunk_type: 'function', line_count: 1, ...over,
} as ChunkNode);
const call = (source: string, target: string, over: Partial<CallEdge> = {}): CallEdge => ({
  source, target, confidence: 1, method: 'static', ...over,
} as CallEdge);
...
describe('buildCallGraphLayout', () => {
  it('keeps only connected chunks and counts disconnected', () => {
    ...
    const ids = r.nodes.filter((n) => !n.isGhost).map((n) => n.id).sort();
  ...
  it('builds ghost nodes from external and only links between existing nodes', () => {
    ...
    const ext: GhostNode[] = [{ id: 'g1', name: 'ext', chunk_type: 'function', module_id: 'm', module_path: 'p/m', direction: 'callee' } as GhostNode];
  ...
  it('assigns curvature 0 to a lone edge and spreads multi-edges', () => {
    ...
    const r2 = buildCallGraphLayout(chunks, [call('a', 'b'), call('a', 'b', { method: 'dyn' })], [], 0, 0);
...
describe('buildSectionTreeLayout', () => {
  ...
  it('creates parent→child edges only when parent present', () => {
    const r = buildSectionTreeLayout([sec('root', null, 0), sec('child', 'root', 1), sec('orphan', 'ghost', 1)], 0, 0);
