...
<script lang="ts">
  ...
  import {
    isClassType, isEnumType, getChunkColors, microNodeLabel, pillWidth,
    PILL_HEIGHT, CLASS_CARD_HEIGHT, GHOST_PILL_WIDTH, GHOST_PILL_HEIGHT,
    getSectionColors,
  } from './types';
  import {
    buildCallGraphLayout, createForceSimulation, edgeBezierPath, type CallGraphLayout,
    buildSectionTreeLayout, createSectionSimulation,
    type SectionLayoutNode, type SectionLayoutEdge, type SectionTreeLayout,
  } from './layout';
  ...
  let {
    chunks, calls, external, moduleName,
    selectedChunkId,
    onselectchunk, onghostclick,
    sectionMode = false, sections = [], documentTitle = '',
    onselectsection,
  }: Props = $props();
  ...
  let hotspotIds: Set<string> = $derived.by(() => layoutResult ? layoutResult.hotspotIds : new Set());
  ...
  function setHoveredNode(id: string | null) {
    ...
    hoverTimer = setTimeout(() => { hoveredNodeId = id; }, 30);
  }
  ...
  let sectionNodes: SectionLayoutNode[] = $derived.by(() => sectionLayout ? sectionLayout.nodes : []);
  let sectionEdges: SectionLayoutEdge[] = $derived.by(() => sectionLayout ? sectionLayout.edges : []);
  ...
  function computeSectionEdgePath(edge: SectionLayoutEdge): string {
    const sid = secEdgeSourceId(edge), tid = secEdgeTargetId(edge);
    const source = sectionNodeById.get(sid);
    const target = sectionNodeById.get(tid);
    if (!source || !target) return '';
    const sx = getSectionNodeX(source), sy = getSectionNodeY(source);
    const tx = getSectionNodeX(target), ty = getSectionNodeY(target);
  ...
  function setHoveredSection(id: string | null) {
    ...
    sectionHoverTimer = setTimeout(() => { hoveredSectionId = id; }, 30);
  }
  ...
  function initSectionLayout() {
    ...
    sectionSim = createSectionSimulation(
      sectionLayout.nodes,
      sectionLayout.edges,
      cx, cy,
      () => {
        ...
        svg.selectAll<SVGGElement, unknown>('.section-node')
          .each(function() {
        ...
        svg.selectAll<SVGPathElement, unknown>('.sec-edge-path')
          .each(function() {
            const el = d3.select(this);
        ...
        svg.selectAll<SVGCircleElement, unknown>('.sec-edge-dot')
          .each(function() {
            const el = d3.select(this);
            const tid = el.attr('data-target');
            const tn = nodeById.get(tid);
            if (tn) { el.attr('cx', tn.x); el.attr('cy', tn.y); }
          });
  ...
  function sectionDrag(el: SVGGElement, nodeId: string) {
    ...
    return {
      destroy() { d3.select(el).on('.drag', null); }
    };
  }
  ...
  function initLayout() {
    ...
    simulation = createForceSimulation(
      ...
      () => {
        ...
        svg.selectAll<SVGGElement, unknown>('.micro-node')
          .each(function() {
            const el = d3.select(this);
        ...
        svg.selectAll<SVGPathElement, unknown>('.edge-path')
          .each(function() {
            const el = d3.select(this);
            const sid = el.attr('data-source');
            const tid = el.attr('data-target');
        ...
        svg.selectAll<SVGCircleElement, unknown>('.edge-dot')
          .each(function() {
            const el = d3.select(this);
            const tid = el.attr('data-target');
            const tn = nodeById.get(tid);
  ...
  function applyCallGraphHover() {
    ...
    svg.selectAll<SVGPathElement, unknown>('.edge-path')
      ...
      .attr('opacity', function() {
        ...
        const el = d3.select(this);
        const sid = el.attr('data-source'), tid = el.attr('data-target');
    ...
    svg.selectAll<SVGCircleElement, unknown>('.edge-dot')
      ...
      .attr('opacity', function() {
        ...
        const isConn = svg.selectAll(`.edge-path[data-target="${tid}"][data-source="${hid}"]`).size() > 0
  ...
  function applySectionTreeHover() {
    ...
    svg.selectAll<SVGCircleElement, unknown>('.sec-edge-dot')
      ...
      .attr('opacity', function() {
        ...
        const el = d3.select(this);
        const tid = el.attr('data-target');
        const isConn = svg.selectAll(`.sec-edge-path[data-target="${tid}"][data-source="${hid}"]`).size() > 0
  ...
  $effect(() => { hoveredNodeId; hoveredConnectedIds; untrack(() => applyCallGraphHover()); });
  $effect(() => { hoveredSectionId; hoveredSectionConnectedIds; untrack(() => applySectionTreeHover()); });
  ...
  onDestroy(() => {
    ...
    if (sectionSim) { sectionSim.stop(); sectionSim = null; }
    zoomBehavior = null;
  });
</script>
...
{#if sectionMode}
  <div class="relative w-full h-full overflow-hidden" bind:this={containerEl}>
        ...
        <g class="section-nodes">
          {#each sectionNodes as node (node.id)}
            ...
            >
              ...
              {/if}
              <title>{node.heading} (depth {node.depth}){hasExplains ? ` — ${explainsCount} explains` : ''}</title>
            </g>
          {/each}
        </g>
      </g>
    ...
    <div class="absolute top-3 left-3 z-10 rounded-lg border border-white/[0.06] bg-[hsl(220_25%_7%/0.85)] backdrop-blur-sm px-3 py-2.5 min-w-[110px]">
      <span class="font-mono text-[10px] font-semibold tracking-widest uppercase text-muted-foreground block mb-1.5">Section Tree</span>
      <!-- D0 Rose — Root -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #3D1428; border: 1.2px solid #F472B6;"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Root</span>
      </div>
      <!-- D1 Teal — Level 1 -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #0D3331; border: 1.2px solid #2DD4BF;"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Level 1</span>
      </div>
      <!-- D2 Blue — Level 2 -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #172554; border: 1.2px solid #60A5FA;"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Level 2</span>
      </div>
      <!-- D3 Violet — Level 3+ -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #1E1B4B; border: 1.2px solid #A78BFA;"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Level 3+</span>
      </div>
      <div class="h-px bg-white/[0.06] my-1.5"></div>
      <!-- Amber — Has Explains -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #2A1F05; border: 1.2px solid #FBBF24;"></span>
      ...
      <div class="flex items-center gap-2">
        <svg class="w-4 h-2.5 shrink-0" viewBox="0 0 20 10">
          <path d="M10,0 C10,5 17,5 17,10" stroke="#F472B6" stroke-width="1.2" fill="none" opacity="0.4" />
...
{:else}
  <div class="relative w-full h-full overflow-hidden" bind:this={containerEl}>
        ...
        <g class="nodes">
          {#each graphNodes as node (node.id)}
            ...
            >
              ...
              {/if}
              <title>{node.isGhost ? `Ghost: ${label} (${node.ghostModulePath ?? ''})` : `${node.chunk_type}: ${label} (${node.line_count} lines)`}</title>
            </g>
          {/each}
        </g>
      </g>
    ...
    <!-- Legend overlay (HTML) -->
    <div class="absolute top-3 left-3 z-10 rounded-lg border border-white/[0.06] bg-[hsl(220_25%_7%/0.85)] backdrop-blur-sm px-3 py-2.5 min-w-[110px]">
      <span class="font-mono text-[10px] font-semibold tracking-widest uppercase text-muted-foreground block mb-1.5">Call Graph</span>
      <!-- Function -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #0D3331; border: 1.2px solid #2DD4BF;"></span>
      ...
      <!-- Class / Struct -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-3.5 h-2.5 shrink-0 rounded-[2px]" style="background: #2D0F1E; border: 1px solid #FB7185; border-left: 2px solid rgba(251,113,133,0.4);"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Class / Struct</span>
      </div>
      <!-- Hotspot -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: #0D3331; border: 1.5px solid #2DD4BF; box-shadow: 0 0 4px rgba(45,212,191,0.3);"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Hotspot</span>
      </div>
      <!-- Enum -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-3 h-2.5 shrink-0 rounded-[2px]" style="background: #2A1F05; border: 1px solid #FBBF24;"></span>
        <span class="text-[10px] text-muted-foreground font-mono">Enum</span>
      </div>
      <!-- Ghost -->
      <div class="flex items-center gap-2 mb-1">
        <span class="w-3 h-2 shrink-0 rounded-full" style="background: #1E293B; border: 1px dashed #64748B; opacity: 0.5;"></span>
...
<style>
  ...
  .edge-path { opacity: 0.1; }
  .edge-path.hot { opacity: 0.18; }
  .edge-path.external { opacity: 0.06; }
  .sec-edge-path { opacity: 0.15; }
  .edge-dot { opacity: 0.08; }
  .edge-dot.hot { opacity: 0.12; }
  .edge-dot.external { opacity: 0.04; }
  .sec-edge-dot { opacity: 0.1; }
</style>
