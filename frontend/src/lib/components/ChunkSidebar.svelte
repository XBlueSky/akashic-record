...
>
	...
	{#if sectionMode && selectedSection}
		<div class="space-y-3 p-3">
			...
			{#if selectedSection.tags && selectedSection.tags.length > 0}
				<div class="flex flex-wrap gap-1">
					{#each selectedSection.tags as tag (tag)}
						<span
							class="rounded-full bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 text-[10px] font-mono text-cyan-400"
						>
							{tag}
						</span>
					{/each}
			...
			<!-- Content -->
			<div
				class="rounded-md bg-black/20 p-3 font-mono text-xs text-slate-300 leading-relaxed max-h-[400px] overflow-y-auto whitespace-pre-wrap"
			>
			...
			{#if selectedSection.explains.length > 0}
				<div class="border-t border-amber-500/20 pt-3">
					...
					{#each selectedSection.explains as target (target.chunk_id)}
						...
						>
							<span
								class="h-2 w-2 shrink-0 rounded-full {target.confidence >= 1.0
									? 'bg-amber-400'
				...
				{#if data.notes.length > 0}
					<div class="space-y-2">
						{#each data.notes as note (note.id)}
							...
							<div
								class="rounded-md border px-3 py-2 transition-colors {onselectnote
									? 'cursor-pointer hover:bg-white/5'
									: ''}"
								style="border-color: {categoryColor(note.category)}40; background: {categoryColor(
									note.category,
								)}08;"
							...
							>
								<div class="flex items-center gap-2 mb-1">
									...
									></span>
									<span
										class="text-[10px] font-bold tracking-wide uppercase font-mono"
							...
							<div class="ml-4 space-y-0.5">
								{#each group.chunks as chunk (chunk.id)}
									<div data-chunk-id={chunk.id}>
										<button
											class="w-full flex items-center gap-2 py-1 px-2 rounded hover:bg-white/5 cursor-pointer bg-transparent border-none text-left transition-colors {focusChunkId ===
											chunk.id
										...
										>
											...
											{#if chunk.notes.length > 0}
												...
												{#if chunk.notes.length > 1}
													<span class="text-[9px] text-amber-400 font-mono ml-0.5"
														>{chunk.notes.length}</span
													>
												{/if}
											{/if}
										</button>
										...
										{#if expandedChunk === chunk.id}
											<div class="ml-5 mt-1 mb-2 space-y-2">
															...
															>
																<span
																	...
																	style="color: {categoryColor(
																		...
																		note.category,
																	)}15; border: 1px solid {categoryColor(note.category)}30;"
																>
												...
												{/if}
												<pre
													class="custom-scrollbar text-xs font-mono text-foreground/80 bg-black/30 rounded-md p-3 max-h-96 overflow-y-auto whitespace-pre-wrap break-words border border-border">{chunk.content}</pre>
											</div>
										{/if}
									</div>
								{/each}
