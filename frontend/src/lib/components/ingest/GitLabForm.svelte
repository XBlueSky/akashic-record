<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    if (!trimmed) { error = get(t)("ingest.gitlab.urlRequired"); return; }
    if (!trimmed.startsWith("http")) { error = get(t)("ingest.gitlab.urlInvalid"); return; }
    submitting = true;
    try {
  ...
  async function loadBranches(repo: string) {
    ...
    try {
      ...
      if (lastFetchedRepo !== repo) return;
      branchesError =
        err instanceof Error ? err.message : get(t)("ingest.gitlab.branchError");
      gitRef = "main";
    } finally {
  ...
  <Card.Root class="border-border/50 bg-card/80 backdrop-blur-xl">
    ...
    <Card.Content class="space-y-4">
      ...
      <div class="space-y-1.5">
        ...
        {#if branchesLoading}
          <div class="flex items-center gap-2 rounded-md border border-border px-3 py-2 font-mono text-xs text-muted-foreground">
            <Loader size={12} class="animate-spin" /> {$t("ingest.gitlab.branchLoading")}
          </div>
