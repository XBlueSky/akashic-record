<script lang="ts">
	...
	function handleSubmit() {
		...
		if (!trimmed) {
			error = get(t)("ingest.website.urlRequired");
			return;
		}
		...
		try {
			...
			error = get(t)("ingest.website.urlInvalid");
			return;
		}
		onSubmit({
			url: trimmed,
		...
		<Card.Content class="space-y-4">
			...
			{#if siteName !== "—"}
				<div class="space-y-1.5">
					<div class="rounded-md border border-primary/12 bg-primary/6 px-2.5 py-2 text-xs">
						...
						<span
							class="ml-1.5 inline-block rounded bg-white/5 px-2 py-0.5 font-mono text-xs text-blue-400"
							>{siteName}</span
						>
					</div>
			...
			<div class="space-y-1.5">
				...
				<ToggleGroup type="single" bind:value={crawlDepth} variant="outline" class="w-full">
					<ToggleGroupItem value="1" class="flex-1 font-mono text-xs"
						>{$t("ingest.website.depth1")}</ToggleGroupItem
					>
					<ToggleGroupItem value="2" class="flex-1 font-mono text-xs"
						>{$t("ingest.website.depth2")}</ToggleGroupItem
					>
					<ToggleGroupItem value="3" class="flex-1 font-mono text-xs"
						>{$t("ingest.website.depth3")}</ToggleGroupItem
					>
				</ToggleGroup>
			</div>
