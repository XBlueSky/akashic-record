<script lang="ts">
	...
	let progressPercent = $derived(
		job && job.total_files > 0 ? Math.round((job.processed_files / job.total_files) * 100) : 0,
	);
			...
			{#if job}
				...
				<div class="flex flex-col gap-2.5">
					{#each STAGES as stage (stage.key)}
						...
						<div
							class="flex items-center gap-2.5 font-mono text-xs tracking-[0.04em] transition-colors {state ===
							'done'
								? 'text-emerald-500'
								: state === 'active'
									? 'text-primary'
						...
						>
							...
							{#if state === "active" && job.total_files > 0}
								<span class="text-secondary-foreground">
									{$t("ingest.progress.files", {
										values: { processed: job.processed_files, total: job.total_files },
									})}
								</span>
							{/if}
						</div>
				...
				{#if job.status === "done"}
					<div
						class="flex items-center gap-2 rounded-md border border-emerald-500/20 bg-emerald-500/8 p-2.5 font-mono text-xs text-emerald-500"
					>
						<Check size={14} />
				...
				{#if job.status === "failed"}
					<div
						class="flex items-center gap-2 rounded-md border border-destructive/20 bg-destructive/8 p-2.5 font-mono text-xs text-destructive"
					>
						<X size={14} />
