...
<div class="mx-auto w-full max-w-3xl">
	...
	{#if loading}
		<p
			class="py-12 text-center font-mono text-sm text-muted-foreground"
	...
	{:else if error}
		<p
			class="py-12 text-center font-mono text-sm text-destructive"
			transition:fade={{ duration: 150 }}
		>
			{error}
		</p>
		...
		<div transition:fade={{ duration: 150 }}>
			...
			<div class="mb-8 rounded-md border border-zinc-800 bg-zinc-900/50 p-5">
				<div class="flex items-start justify-between gap-4">
					...
					<span
						class="shrink-0 rounded border px-2 py-0.5 font-mono text-xs {STATUS_BADGES[
							detail.saga.status
				...
				<div class="relative ml-4 border-l border-zinc-700 pl-6">
					{#each detail.timeline as entry (entry.uuid)}
						<div class="relative mb-6 last:mb-0 {isSuperseded(entry) ? 'opacity-50' : ''}">
							<!-- Dot -->
							<div
								class="absolute -left-[calc(1.5rem+0.3125rem)] top-1 h-2.5 w-2.5 rounded-full {isSuperseded(
									entry,
								)
									? 'bg-zinc-600'
									: (DOT_COLORS[entry.category] ?? 'bg-blue-400')}"
							></div>
							...
							<!-- Category badge -->
							<span
								class="mt-1 inline-block rounded border px-1.5 py-0.5 font-mono text-[10px] uppercase {CATEGORY_COLORS[
									entry.category
							...
							{#if entry.title}
								<h3
									class="mt-1.5 text-sm font-medium {isSuperseded(entry)
