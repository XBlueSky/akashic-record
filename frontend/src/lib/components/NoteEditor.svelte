<script lang="ts">
	...
	let { noteUuid, repoName = "", onclose, onsaved }: Props = $props();
		...
		try {
			...
			if (!isAuthError(e)) {
				const msg =
					e instanceof Error
						? e.message
						: String($t("noteEditor.saveFailed") ?? "Failed to save note");
				error = msg;
...
>
	...
	<div class="sticky top-0 z-10 glass border-b border-border px-4 py-3 shrink-0">
		<div class="flex items-center justify-between">
			<h2 class="text-sm font-semibold text-foreground font-mono">
				{$t("noteEditor.editNote") || "Edit Note"}
			</h2>
			<button
	...
	{:else}
		<div class="flex flex-col flex-1 overflow-y-auto custom-scrollbar">
			<div class="px-4 py-3 space-y-3 flex flex-col flex-1">
				...
				<div class="space-y-1">
					<label
						for="note-title"
						class="text-xs font-mono text-muted-foreground uppercase tracking-wide"
						>{$t("noteEditor.title") || "Title"}</label
					>
					<input
						id="note-title"
						type="text"
				...
				<div class="space-y-1">
					<label
						for="note-category"
						class="text-xs font-mono text-muted-foreground uppercase tracking-wide"
						>{$t("noteEditor.category") || "Category"}</label
					>
					<select
						id="note-category"
						bind:value={category}
					...
					>
						{#each CATEGORIES as cat (cat)}
							<option value={cat} class="bg-[#09090b] text-foreground"
				...
				<div class="space-y-1">
					...
					>
						Issue Ref
						<span
							class="text-[10px] text-slate-600 normal-case tracking-normal ml-1"
							title="Saga assignment is handled automatically via MCP tools (save_note)"
							>MCP only</span
						>
					</label>
					<input
				...
				<div class="space-y-1">
					<div class="flex items-center justify-between">
						<label
							for="note-summary"
							class="text-xs font-mono text-muted-foreground uppercase tracking-wide"
							>{$t("noteEditor.summary") || "Summary"}</label
						>
						<span
							class="text-[10px] font-mono {summaryLen > 100
								? 'text-red-400'
								: 'text-muted-foreground'}">{summaryLen}/100</span
						>
					</div>
					<input
						id="note-summary"
				...
				<div class="flex gap-1 border-b border-border">
					<button
						class="px-3 py-1.5 text-xs font-mono cursor-pointer bg-transparent border-none transition-colors {activeTab ===
						'write'
							? 'text-foreground border-b-2 border-b-blue-500'
							: 'text-muted-foreground hover:text-foreground'}"
						onclick={() => (activeTab = "write")}>{$t("noteEditor.write") || "Write"}</button
					>
					<button
						class="px-3 py-1.5 text-xs font-mono cursor-pointer bg-transparent border-none transition-colors {activeTab ===
						'preview'
							? 'text-foreground border-b-2 border-b-blue-500'
							: 'text-muted-foreground hover:text-foreground'}"
						onclick={() => (activeTab = "preview")}>{$t("noteEditor.preview") || "Preview"}</button
					>
				</div>
				...
				<div class="flex-1 min-h-[200px] flex flex-col">
					{#if activeTab === "write"}
						<textarea
							...
							placeholder={String(
								$t("noteEditor.contentPlaceholder") ?? "Write your note in Markdown…",
							)}></textarea>
					{:else}
						<div
							class="flex-1 border border-border rounded-md px-3 py-2 text-sm text-foreground overflow-y-auto note-prose"
						>
							{#if content.trim()}
							...
							{:else}
								<p class="text-muted-foreground italic">
									{$t("noteEditor.nothingToPreview") || "Nothing to preview."}
								</p>
							{/if}
						</div>
					{/if}
				...
				<div class="space-y-1.5">
					<label
						for="note-tag-input"
						class="text-xs font-mono text-muted-foreground uppercase tracking-wide"
						>{$t("noteEditor.tags") || "Tags"}</label
					>
					<div class="flex flex-wrap gap-1.5 items-center">
						{#each tags as tag (tag)}
							<span
								class="inline-flex items-center gap-1 bg-white/5 border border-border rounded-full px-2.5 py-0.5 text-xs font-mono text-foreground"
							>
								{tag}
								<button
		...
		<!-- Footer -->
		<div
			class="sticky bottom-0 glass border-t border-border px-4 py-3 flex items-center gap-2 shrink-0"
		>
			...
			<button
				...
				onclick={onclose}>{$t("noteEditor.cancel") || "Cancel"}</button
			>
			<button
