<script lang="ts">
	...
	async function handleSync(name: string) {
		...
		try {
			...
			toast.push(`Sync failed for ${name}: ${e instanceof Error ? e.message : String(e)}`, "error");
		} finally {
		...
		{#if data}
			<div class="flex items-center gap-4 font-mono text-[10px]">
				...
				<span class="text-green-400/70"
					>{$t("sources.healthy", { values: { count: data.healthy } })}</span
				>
				{#if data.stale > 0}<span class="text-amber-400/70"
						>{$t("sources.stale", { values: { count: data.stale } })}</span
					>{/if}
				{#if data.failed > 0}<span class="text-red-400/70"
						>{$t("sources.failed", { values: { count: data.failed } })}</span
					>{/if}
				{#if data.ingesting > 0}<span class="text-blue-400/70"
						>{$t("sources.active", { values: { count: data.ingesting } })}</span
					>{/if}
			</div>
		{/if}
		<button
			...
			onclick={() => goto("/ingest")}>{$t("sources.addSource")}</button
		>
	</div>
		...
		<table class="w-full font-mono text-[11px]">
			<thead>
				<tr class="border-b border-white/5">
					<th class="text-left text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3"
						>{$t("sources.colSource")}</th
					>
					<th class="text-left text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3"
						>{$t("sources.colStatus")}</th
					>
					<th class="text-left text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3"
						>{$t("sources.colBranch")}</th
					>
					<th class="text-right text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3"
						>{$t("sources.colChunks")}</th
					>
					<th class="text-right text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3"
						>{$t("sources.colNotes")}</th
					>
					<th class="text-left text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3"
						>{$t("sources.colSynced")}</th
					>
					<th class="text-right text-[10px] text-slate-600 uppercase tracking-wider pb-2 px-3 w-24"
					></th>
				</tr>
			</thead>
			<tbody>
				{#each Array(4) as _, i (i)}
					<tr class="border-b border-white/[0.03]">
						<td class="px-3 py-2.5"
							><div class="h-3 w-32 bg-white/[0.04] rounded animate-pulse"></div></td
						>
						<td class="px-3 py-2.5"
							><div class="h-3 w-14 bg-white/[0.04] rounded animate-pulse"></div></td
						>
						<td class="px-3 py-2.5"
							><div class="h-3 w-12 bg-white/[0.04] rounded animate-pulse"></div></td
						>
						<td class="px-3 py-2.5 text-right"
							><div class="h-3 w-8 bg-white/[0.04] rounded animate-pulse ml-auto"></div></td
						>
						<td class="px-3 py-2.5 text-right"
							><div class="h-3 w-6 bg-white/[0.04] rounded animate-pulse ml-auto"></div></td
						>
						<td class="px-3 py-2.5"
							><div class="h-3 w-14 bg-white/[0.04] rounded animate-pulse"></div></td
						>
		...
		<table class="w-full font-mono text-[11px]">
			...
			<tbody>
				{#each data.sources as source (source.name)}
					<tr
						class="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors group {source.status ===
						'failed'
							? 'bg-red-500/[0.03]'
							: ''}"
					>
							...
							<div class="flex items-center gap-1.5">
								{#if source.status === "ingesting"}
									...
									<span class="text-blue-400"
										>{source.active_job?.status ?? $t("sources.statusIngesting")}</span
									>
								...
								{:else}
									<span class="inline-block w-[6px] h-[6px] rounded-full bg-green-400 shrink-0"
									></span>
							...
							{#if source.status === "ingesting" && source.active_job?.total_files}
								<div class="mt-1.5 h-[2px] w-full bg-white/5 rounded-full overflow-hidden">
									<div
										...
										style="width: {((source.active_job.processed_files ?? 0) /
											source.active_job.total_files) *
											100}%"
									></div>
								</div>
							{/if}
						...
						<td class="px-3 py-2.5 text-slate-600">
							{#if source.status === "failed"}
								<span class="text-red-400/70 text-[10px]"
									>{source.last_error?.slice(0, 40) ?? "failed"}</span
								>
							{:else}
						...
						<td class="px-3 py-2.5 text-right">
							<div
								class="flex items-center justify-end gap-2 transition-opacity {source.status ===
									'failed' ||
								source.status === 'stale' ||
							...
							>
								...
								{:else if source.status === "failed"}
									{#if source.can_resume}
										<button
											class="text-blue-400 hover:text-blue-300 bg-transparent border-none cursor-pointer p-0"
											title={$t("sources.actionResume")}
											onclick={() => handleResume(source.name)}><Play size={12} /></button
										>
									{/if}
									<button
										class="text-slate-500 hover:text-blue-400 bg-transparent border-none cursor-pointer p-0"
										title={$t("sources.actionRetry")}
										onclick={() => handleSync(source.name)}><RefreshCw size={12} /></button
									>
									<button
										class="text-slate-500 hover:text-red-400 bg-transparent border-none cursor-pointer p-0"
										title={$t("sources.actionDelete")}
										onclick={() => openDeleteDialog(source)}><X size={12} /></button
									>
								{:else}
									<button
										class="text-slate-500 hover:text-blue-400 bg-transparent border-none cursor-pointer p-0"
										title={$t("sources.actionSync")}
										onclick={() => handleSync(source.name)}><RefreshCw size={12} /></button
									>
									<button
										class="text-slate-500 hover:text-red-400 bg-transparent border-none cursor-pointer p-0"
...
<AlertDialog.Root bind:open={showDeleteDialog}>
	<AlertDialog.Content
		class="bg-[#0f1729]/80 backdrop-blur-xl border border-white/10 shadow-[0_8px_40px_-10px_rgba(0,0,0,0.6)]"
	>
		<AlertDialog.Header>
			...
			<AlertDialog.Description class="text-slate-400">
				{#if deleteTarget}
					{$t("sources.deleteDesc", {
						values: {
							name: deleteTarget.name,
							chunks: deleteTarget.chunk_count,
							notes: deleteTarget.note_count,
						},
					})}
				{/if}
		...
		<div class="space-y-2 py-2">
			<label
				...
				class="font-mono text-[10px] font-semibold tracking-widest uppercase text-slate-500"
			>
