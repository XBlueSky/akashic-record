...
{#if targets.length > 0}
  <section class="mt-8" data-testid="related-code">
    <h2 class="mb-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{$t("docs.related.title")}</h2>
    <ul class="space-y-1">
      {#each targets as e (e.chunk_id)}
        <li>
          ...
          >
            <span class="font-mono text-[12px] text-foreground group-hover:text-primary">{e.chunk_name}</span>
            <span class="truncate font-mono text-[10px] text-muted-foreground">{e.module_path}</span>
            <span class="ml-auto font-mono text-[10px] text-muted-foreground">{Math.round(e.confidence * 100)}%</span>
          </a>
        </li>
      {/each}
    </ul>
