...
{#if toc.length > 0}
  <aside class="hidden w-48 shrink-0 xl:block" data-testid="docs-toc">
    <div class="sticky top-4">
      <div class="mb-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{$t("docs.toc")}</div>
        ...
        {#each toc as e (e.id)}
          <li>
            <a
              ...
              class="-ml-px block border-l-2 py-0.5 text-[12px] leading-5 transition-colors duration-150 {e.level === 3
                ? 'pl-6'
                : 'pl-3'} {active === e.id
