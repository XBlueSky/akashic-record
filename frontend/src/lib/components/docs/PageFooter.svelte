...
<footer class="mt-10 border-t border-border pt-4" data-testid="page-footer">
  {#if prev || next}
    <div class="mb-4 flex justify-between gap-4 text-[13px]">
      {#if prev}
        <a href={hrefFor(prev)} class="group text-muted-foreground transition-colors duration-150 hover:text-foreground">
      ...
      {:else}<span></span>{/if}
      {#if next}
        <a href={hrefFor(next)} class="group text-right text-muted-foreground transition-colors duration-150 hover:text-foreground">
