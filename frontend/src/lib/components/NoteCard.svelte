<script lang="ts">
  ...
  let { note, highlight = false, canEdit = false, canDelete = false, onedit, ondelete }: {
    note: Note;
    highlight?: boolean;
    canEdit?: boolean;
    canDelete?: boolean;
...
<Card.Root
  id="note-{note.uuid}"
  class="rounded-xl border !py-0 transition-all hover:bg-white/[0.03] {highlight ? 'border-amber-500/50 bg-amber-500/[0.06] ring-1 ring-amber-500/30' : 'border-white/10 bg-white/[0.02]'}"
...
>
  ...
  <Card.Header class="flex flex-row flex-wrap items-center justify-between gap-1.5 px-3 py-1.5">
    <div class="flex items-center gap-1.5 flex-wrap">
      <Badge
        variant="outline"
        class="rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide {categoryVariant[note.category]}"
      >
    ...
    <div class="flex items-center gap-2 text-xs text-slate-500">
      ...
      {#if canEdit}
        <button
          ...
          title="Edit note"
          onclick={(e) => { e.stopPropagation(); onedit?.(note.uuid); }}
        >
          <Pencil size={13} />
        </button>
      {/if}
      {#if canDelete}
        <button
          ...
          title="Delete note"
          onclick={(e) => { e.stopPropagation(); ondelete?.(note.uuid); }}
        >
          <Trash2 size={13} />
        </button>
      {/if}
    ...
    <div transition:slide={{ duration: 200 }}>
        ...
        <div class="px-3 py-1 ml-5">
          <ul class="list-none space-y-0">
            {#each note.facts as fact (fact)}
              <li class="relative pl-3 text-xs leading-normal text-slate-400 before:absolute before:left-0 before:text-slate-600 before:content-['•']">
                {fact}
              </li>
            {/each}
          </ul>
      ...
      {#if note.related_files && note.related_files.length > 0}
        <div class="flex flex-wrap items-center gap-1 border-t border-white/5 px-3 py-1.5 ml-5">
          <span class="mr-1 font-mono text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{$t("repo.files")}</span>
          {#each note.related_files as file (file)}
            <Badge variant="outline" class="border-blue-500/20 bg-blue-500/10 font-mono text-[10px] text-blue-400">
              {file}
            </Badge>
          {/each}
        </div>
      ...
      {#if note.related_symbols && note.related_symbols.length > 0}
        <div class="flex flex-wrap items-center gap-1 border-t border-white/5 px-3 py-1.5 ml-5">
          <span class="mr-1 font-mono text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{$t("repo.symbols")}</span>
          {#each note.related_symbols as sym (sym)}
            <Badge variant="outline" class="border-green-500/20 bg-green-500/10 font-mono text-[10px] text-green-400">
              {sym}
            </Badge>
          {/each}
        </div>
  ...
  {#if note.tags && note.tags.length > 0}
    <Card.Footer class="flex flex-wrap gap-1 border-t border-white/5 px-3 !pt-1.5 !pb-1.5">
      {#each note.tags as tag (tag)}
        <Badge variant="outline" class="rounded-full border-white/10 bg-white/[0.03] px-1.5 py-0 text-[10px] text-slate-500 font-mono">
          {tag}
        </Badge>
      {/each}
    </Card.Footer>
...
<style>
  ...
  :global(.note-prose h1) { font-size: 1.1em; font-weight: 700; margin: 0.4em 0 0.15em; color: #e2e8f0; }
  :global(.note-prose h2) { font-size: 1em; font-weight: 600; margin: 0.35em 0 0.1em; color: #e2e8f0; }
  :global(.note-prose h3) { font-size: 0.95em; font-weight: 600; margin: 0.3em 0 0.1em; color: #cbd5e1; }
  :global(.note-prose ul) { list-style: disc; padding-left: 1.25em; margin: 0.15em 0; }
  :global(.note-prose ol) { list-style: decimal; padding-left: 1.25em; margin: 0.15em 0; }
  :global(.note-prose li) { margin: 0; }
  :global(.note-prose blockquote) { border-left: 2px solid #475569; padding-left: 0.6em; margin: 0.2em 0; color: #94a3b8; font-style: italic; }
  :global(.note-prose pre) { overflow-x: auto; border-radius: 0.25rem; border: 1px solid hsl(var(--border)); background: #09090b; padding: 0.5rem; margin: 0.25em 0; font-size: 0.8em; line-height: 1.4; }
  :global(.note-prose code) { font-family: ui-monospace, monospace; }
  :global(.note-prose :not(pre) > code) { background: hsl(var(--muted)); padding: 0.1em 0.3em; border-radius: 0.2rem; font-size: 0.85em; color: #60a5fa; }
  :global(.note-prose p) { margin: 0.15em 0; }
  :global(.note-prose p:first-child) { margin-top: 0; }
  :global(.note-prose p:last-child) { margin-bottom: 0; }
  :global(.note-prose a) { color: #60a5fa; text-decoration: underline; }
  :global(.note-prose table) { border-collapse: collapse; margin: 0.25em 0; width: 100%; }
  :global(.note-prose th, .note-prose td) { border: 1px solid #334155; padding: 0.2em 0.5em; font-size: 0.85em; }
