...
<div class="flex h-[calc(100vh-120px)]">
	...
	<div class="w-70 min-w-70 overflow-y-auto border-r border-border/50 bg-card/50 backdrop-blur-md">
		<h3
			class="px-3.5 pt-3 pb-2 font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground"
		>
			{$t("modules.title")}
		</h3>
		{#if loadingModules}
	...
	<div class="flex-1 overflow-y-auto p-3 px-5">
		...
		{:else}
			<div class="mb-4">
				<h3
					class="px-3.5 pb-1 font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground"
				>
					{selectedModule.path}
				</h3>
							...
							<Card.Root class="overflow-hidden border-border/50 bg-card/50">
								...
								>
									<div class="flex items-center gap-2">
										<Badge
											variant="outline"
											class="text-[10px] font-bold uppercase tracking-wide {chunkTypeClass(
												chunk.chunk_type,
											)}"
										>
											{chunk.chunk_type}
									...
									<svg
										class="h-3 w-3 text-muted-foreground transition-transform duration-200 {isExpanded(
											chunk.id,
										)
											? 'rotate-180'
											: ''}"
								...
								{#if chunk.signature}
									<div
										class="border-t border-border/30 px-3 py-1.5 font-mono text-xs text-secondary-foreground"
									>
										{chunk.signature}
									</div>
								{/if}
								...
								<Collapsible.Content>
									<pre
										class="m-0 overflow-x-auto bg-zinc-950 p-3 font-mono text-xs leading-relaxed text-foreground"><code
											>{chunk.content}</code
										></pre>
								</Collapsible.Content>
							</Card.Root>
