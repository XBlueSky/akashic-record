<script lang="ts">
	...
	async function handleWebsiteSubmit(data: {
		url: string;
		crawl_depth: number;
		url_pattern: string;
	}) {
...
<div class="flex h-full min-h-[400px] w-full flex-col items-center justify-center">
	{#if submitError}
		<div
			class="mb-4 max-w-[480px] rounded-md border border-destructive/25 bg-destructive/10 px-3.5 py-2.5 text-sm text-destructive"
		>
			{submitError}
		</div>
	{/if}
	...
	{:else if !auth.user}
		<Card.Root class="max-w-[420px] border-border/50 bg-card/80 text-center backdrop-blur-xl">
			<Card.Content class="space-y-6 px-10 py-12">
				...
				<h2
					class="text-2xl font-bold tracking-[0.18em] text-primary drop-shadow-[0_0_20px_rgba(59,130,246,0.4)]"
				>
					{$t("ingest.auth.title")}
				</h2>
