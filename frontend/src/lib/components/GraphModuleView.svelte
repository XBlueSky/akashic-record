...
<script lang="ts">
	...
	function applyHoverOpacity() {
		...
		svg
			...
			.attr("opacity", (d) =>
				!hoveredNode
					? 0.3 + (d?.confidence ?? 0.8) * 0.35
					: (d?.source as SimNode)?.id === hoveredNode || (d?.target as SimNode)?.id === hoveredNode
						? 1
						: 0.06,
			)
			.attr("stroke-width", (d) =>
	...
	async function enterDrilldown(node: SimNode) {
		...
		if (highlightChunkId && drillData) {
			...
			if (chunkExists) {
				tick().then(() => {
					selectedChunkId = highlightChunkId;
				});
			}
		}
	}
		...
		try {
			if (isDocMode) {
				...
				const simLinks: SimLink[] = data.edges.map((e: ImportEdge) => ({
					...
					target: e.target,
					curvature: 0,
					edgeType: (e.source === repoNodeId
						? "HAS_MODULE"
						: "IMPORTS_FROM") as SimLink["edgeType"],
					confidence:
						e.source === repoNodeId
							? 1.0
							: (((e as unknown as Record<string, unknown>)["confidence"] as number) ?? 0.8),
				}));
			...
			if (myGeneration === loadGeneration) {
				...
				fetchGodNodes(repo)
					.then((res) => {
						...
						godNodeIds = new Set(
							res.god_nodes
								.filter((g: GodNode) => g.node_type === "module")
								.map((g: GodNode) => g.id),
						);
					})
					.catch(() => {});
	...
	function initSimulation() {
		...
		simulation = d3
			...
			.force(
				"collide",
				d3.forceCollide<SimNode>().radius((d) => nodeSize(d) + 15),
			)
			...
			.on("tick", () => {
				...
				svg.selectAll<SVGPathElement, SimLink>(".edges path").data(links).attr("d", edgePath);
				const hullByGroup = new Map<SagaGroup, [number, number][] | null>(
					sagaGroups.map((g) => [g, computeSagaHull(g)]),
				);
				svg
					.selectAll<SVGPolygonElement, SagaGroup>(".saga-hulls polygon")
					.data(sagaGroups)
					.attr("points", (g) => {
				...
				svg
					.selectAll<SVGTextElement, SagaGroup>(".saga-hulls text")
					.data(sagaGroups)
					.attr("x", (g) => {
						const hull = hullByGroup.get(g);
	...
	$effect(() => {
		if (repoName) loadGraph(repoName);
	});
...
>
	{#if loading}
		<div class="flex flex-col items-center justify-center h-full gap-2">
			<p class="font-mono text-lg font-semibold text-muted-foreground tracking-widest">
				{$t("graph.loading")}
			</p>
	...
	{:else if error}
		<div class="flex flex-col items-center justify-center h-full gap-2">
			<p class="font-mono text-lg font-semibold text-red-500 tracking-widest">
				{$t("graph.error")}
			</p>
	...
	{:else if viewMode === "MACRO"}
		...
		<svg bind:this={macroSvgEl} class="w-full h-full" style="background: transparent;">
			<defs>
				...
				<filter id="glow-strong" x="-50%" y="-50%" width="200%" height="200%">
					...
					<feMerge
						><feMergeNode in="bloom" /><feMergeNode in="blur" /><feMergeNode
							in="SourceGraphic"
						/></feMerge
					>
				</filter>
				...
				<g class="edges">
					{#each links as link, idx (idx)}
						<path
							...
							stroke-dasharray={link.confidence >= 0.9
								? "none"
								: link.confidence >= 0.5
									? "6 4"
									: "2 3"}
					...
					{#each nodes as node (node.id)}
						...
						>
							...
							<circle
								...
								stroke-dasharray={nodeStrokeDash(node)}
								filter={nodeFilter(node)}
							/>
							{#if node.note_count > 0 && !node.isRepo}
								<circle
									cx={nodeSize(node) * 0.7}
									cy={-nodeSize(node) * 0.7}
									r="4"
									fill="#F59E0B"
									class="animate-pulse"
									pointer-events="none"
							...
							<title
								>{node.isRepo
									? node.label
									: isDocMode
										? `${nodeLabel(node)} — ${node.section_count ?? 0} sections, ${node.explains_count ?? 0} explains\nDouble-click to drill down`
										: `${nodeLabel(node)} — ${node.chunk_count} chunks, ${node.note_count} notes\nDouble-click to drill down`}</title
							>
						</g>
					{/each}
				</g>
		...
		<div class="absolute top-4 left-4 z-2">
			{#if legendOpen}
				<div class="glass rounded-lg p-3 min-w-[130px]">
					<button
						class="w-full flex items-center justify-between mb-1.5 cursor-pointer bg-transparent border-none"
						onclick={() => (legendOpen = false)}
					>
						<span
							class="font-mono text-[10px] font-semibold tracking-widest uppercase text-muted-foreground"
							>Legend</span
						>
					...
					<div class="flex items-center gap-2 mb-1">
						<span
							...
							style="background: rgba(59,130,246,0.1); border: 2px solid #3B82F6; box-shadow: 0 0 6px #3B82F6;"
						></span>
					...
					<div class="flex items-center gap-2 mb-1">
						<span
							...
							style="background: {isDocMode ? '#38BDF8' : '#06B6D4'}; box-shadow: 0 0 6px {isDocMode
								? '#38BDF8'
								: '#06B6D4'};"
						></span>
						<span class="text-[10px] text-muted-foreground font-mono"
							>{isDocMode ? "Document" : "Module"}</span
						>
					</div>
					<div class="flex items-center gap-2 mb-1">
						<span class="w-3.5 border-t border-dashed shrink-0" style="border-color: #2DD4BF;"
						></span>
					...
					<div class="flex items-center gap-2">
						...
						<span class="text-[10px] text-muted-foreground font-mono"
							>{$t("graph.connections")}</span
						>
					</div>
				</div>
			{:else}
				<button
					class="glass rounded-lg px-2.5 py-1.5 cursor-pointer bg-transparent border border-white/5 hover:border-white/10 transition-colors"
		...
		<!-- Macro stats badge -->
		<div
			class="absolute bottom-4 left-4 z-2 glass rounded-lg px-3.5 py-2 text-xs text-muted-foreground font-mono"
		>
			{#if isDocMode}
	...
	{:else if viewMode === "MICRO"}
		...
		{#if drillModule}
			<button
				class="absolute top-4 right-4 z-10 glass rounded-lg px-3 py-1.5 cursor-pointer border border-white/10 hover:border-blue-500/30 hover:bg-blue-500/8 active:bg-blue-500/12 transition-all"
				onclick={() => exitDrilldown()}
			>
				<span class="font-mono text-xs font-semibold text-blue-400 tracking-wide"
					>&larr; {$t("graph.backToModules")}</span
				>
			</button>
		{/if}
	{/if}
	...
	{#if drillLoading}
		<div class="absolute inset-0 flex items-center justify-center">
			<p class="font-mono text-sm text-muted-foreground tracking-widest">
				{$t("graph.drillLoading")}
			</p>
		</div>
	{/if}
	...
	{#if fromLabel}
		...
		>
			<span class="font-mono text-xs font-semibold text-amber-400 tracking-wide"
				>&larr; {fromLabel}</span
			>
		</button>
	{/if}
