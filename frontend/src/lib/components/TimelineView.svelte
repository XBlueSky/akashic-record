...
<div class="relative mx-auto w-full max-w-4xl">
	{#if loading && notes.length === 0}
		<p class="py-12 text-center font-mono text-sm text-muted-foreground">
			{$t("timeline.loading")}
		</p>
	{:else if error}
	...
	{#if loading && notes.length > 0}
		<div
			class="absolute inset-0 flex items-center justify-center rounded-md bg-background/80 font-mono text-sm text-muted-foreground backdrop-blur-sm"
		>
			{$t("common.loading")}
		</div>
	{/if}
