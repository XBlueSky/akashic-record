<script lang="ts">
  ...
  import { Loader, Plus, LogOut, LogIn, Search, Globe, GitBranch, PanelTop, User, Waypoints, Settings } from "@lucide/svelte";
  ...
  let isRepoActive = $derived.by(
    () => (name: string): boolean => activeJobs.some((j) => j.repo_name === name)
  );
  ...
  onMount(async () => {
    ...
    try { activeJobs = await fetchActiveJobs(); } catch (e: unknown) {
    ...
    unsubJobUpdate = onEvent("job_update", (raw) => {
      const data = raw as { repo_name: string; status: string; processed_files: number | null; total_files: number | null };
        ...
        if (existing) {
          activeJobs = activeJobs.map((j) =>
            j.repo_name === data.repo_name
              ? { ...j, status: data.status, processed_files: data.processed_files, total_files: data.total_files }
              : j
          );
        } else {
    ...
    unsubReposChanged = onEvent("repos_changed", async () => {
      try { repos = await fetchRepos(); } catch (e: unknown) {
...
<Sidebar.Root>
  ...
  <Sidebar.Header class="px-4 pb-0 pt-4">
    ...
    >
      <span class="flex items-center gap-2">
        <Search size={14} strokeWidth={2} class="text-slate-500 group-hover:text-slate-300 transition-colors" />
        <span class="font-mono text-sm text-slate-500 group-hover:text-slate-300 transition-colors">{$t('sidebar.searchPlaceholder')}</span>
      </span>
      <kbd class="flex items-center justify-center px-1.5 py-0.5 text-[10px] font-mono text-slate-500 bg-black/20 border border-white/10 rounded">⌘K</kbd>
    </button>
  </Sidebar.Header>
    ...
    {#if loading}
      <Sidebar.Group>
        <Sidebar.GroupContent>
          <Sidebar.Menu>
            <Sidebar.MenuItem>
              <Sidebar.MenuButton class="font-mono text-muted-foreground text-xs italic pointer-events-none">
                {$t('sidebar.loading')}
              </Sidebar.MenuButton>
            </Sidebar.MenuItem>
          </Sidebar.Menu>
    ...
    {:else if error}
      <Sidebar.Group>
        <Sidebar.GroupContent>
          <Sidebar.Menu>
            <Sidebar.MenuItem>
              <Sidebar.MenuButton class="font-mono text-destructive text-xs italic pointer-events-none">
                {error}
              </Sidebar.MenuButton>
            </Sidebar.MenuItem>
          </Sidebar.Menu>
                ...
                <Sidebar.MenuItem>
                  ...
                  >
                    {#if isRepoActive(repo.name)}
                      <span class="inline-flex h-3.5 w-3.5 shrink-0 items-center justify-center text-blue-400 animate-pulse">
                        <Loader size={12} strokeWidth={2} class="animate-spin" />
                      </span>
                    {:else}
                      <GitBranch size={14} strokeWidth={1.5} class="w-3.5 h-3.5 shrink-0 text-slate-500 group-hover:text-blue-400 transition-colors" />
                    {/if}
                    {@const parts = splitName(repo.name)}
                    {#if parts}
                      <div class="flex flex-col min-w-0 overflow-hidden" title={repo.name}>
                        <span class="truncate font-mono text-sm text-slate-300 group-hover:text-blue-400 transition-colors">{parts.path}</span>
                        <span class="truncate text-[10px] font-mono text-slate-500 mt-0.5">{parts.host}</span>
                      </div>
                    {:else}
                      <span class="truncate font-mono text-xs tracking-wide" title={repo.name}>{repo.name}</span>
                    {/if}
                  </Sidebar.MenuButton>
                </Sidebar.MenuItem>
              {/each}
              ...
              {#each webRepos as repo (repo.name)}
                <Sidebar.MenuItem>
                  ...
                  >
                    ...
                    {:else}
                      <PanelTop size={14} strokeWidth={1.5} class="w-3.5 h-3.5 shrink-0 text-slate-500 group-hover:text-blue-400 transition-colors" />
                    {/if}
                    {@const parts = splitName(repo.name)}
                    {#if parts}
                      <div class="flex flex-col min-w-0 overflow-hidden" title={repo.name}>
  ...
  <Sidebar.Footer>
    ...
    <div class="flex items-center px-2 py-1">
      <Popover.Root>
        <Popover.Trigger>
          <button class="flex items-center gap-1.5 rounded-md px-2 py-1 text-xs text-muted-foreground transition-colors hover:text-foreground hover:bg-accent cursor-pointer bg-transparent border-none">
        ...
        <Popover.Content align="start" class="min-w-[120px] p-0 bg-[#0f1729]/80 backdrop-blur-xl border border-white/10 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.5)] rounded-lg overflow-hidden">
          <button
            class="flex w-full items-center font-mono text-sm cursor-pointer bg-transparent border-none transition-colors {$locale === 'en' ? 'bg-blue-500/10 text-blue-400 border-l-[3px] border-blue-500 px-2.5 py-2' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5 px-3 py-2'}"
            onclick={() => setLocale('en')}
          >
            EN
          </button>
          <button
            class="flex w-full items-center font-mono text-sm cursor-pointer bg-transparent border-none transition-colors {$locale === 'zh-TW' ? 'bg-blue-500/10 text-blue-400 border-l-[3px] border-blue-500 px-2.5 py-2' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5 px-3 py-2'}"
    ...
    {#if auth.user}
      <div class="flex items-center gap-2 px-2 py-1.5">
        ...
        <button
          ...
          onclick={() => { const u = new URL(page.url); u.searchParams.set('settings', 'tokens'); goto(u.pathname + u.search); }}
        >
    ...
    {:else}
      <div class="flex items-center gap-2 px-2 py-1.5">
        ...
        <span class="flex-1 truncate font-mono text-xs text-slate-600">
          anonymous
        </span>
        <button
