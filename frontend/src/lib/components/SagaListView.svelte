<script lang="ts">
  ...
  function statusBadgeClass(status: string): string {
    switch (status) {
      ...
      case 'resolved': return 'bg-amber-500/15 text-amber-400 border-amber-500/25';
      case 'archived': return 'bg-zinc-500/15 text-zinc-500 border-zinc-500/25';
      default: return 'bg-zinc-500/15 text-zinc-500 border-zinc-500/25';
    }
  }
      ...
      {#each sagas as saga (saga.id)}
        ...
        >
          <div class="flex items-center gap-3 min-w-0">
            <!-- Source type icon -->
            <span
              class="font-mono text-sm shrink-0 {saga.source_type === 'issue' ? 'text-blue-400' : saga.source_type === 'branch' ? 'text-green-400' : 'text-amber-400'}"
            ...
            <!-- Name -->
            <span class="font-mono text-[12px] text-slate-200 truncate group-hover:text-blue-300 transition-colors">
            ...
            <!-- Status badge -->
            <span class="font-mono text-[10px] px-2 py-0.5 rounded border shrink-0 {statusBadgeClass(saga.status)}">
              {saga.status}
            </span>
