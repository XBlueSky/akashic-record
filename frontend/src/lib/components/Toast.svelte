...
{#if toast.items.length > 0}
	<div class="fixed bottom-4 right-4 z-50 flex flex-col-reverse gap-3 max-w-[400px]">
		{#each toast.items as item (item.id)}
			...
			<div
				class="flex items-start gap-3 rounded-lg border border-border/40 border-l-4 bg-background/80 p-4 shadow-lg backdrop-blur-md animate-slide-up {accentMap[
					item.type
				]}"
				role="alert"
			>
				...
				<div class="flex-1 min-w-0">
					...
					{#if item.action}
						<button
							type="button"
							class="mt-1.5 text-sm font-medium underline underline-offset-2 {actionColorMap[
								item.type
							]} transition-colors"
