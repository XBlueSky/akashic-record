...
import { renderDocsMarkdown, rewriteHref, stripFrontmatter, type DocsRenderCtx } from "../markdown.js";
...
const pull: DocsRenderCtx = { ...push, indexDir: "docs", indexPath: "docs/README.md", pageFullKey: "docs/guide/setup.md" };
...
describe("renderDocsMarkdown", () => {
  it("renders headings with github-slugger ids and collects h2/h3 toc", () => {
    const { html, toc } = renderDocsMarkdown("# Top\n\n## Quick Start\n\n### Sub Step\n\n#### deep", push);
  ...
  it("sanitizes: no inline svg, no script", () => {
    const { html } = renderDocsMarkdown('<svg onload="x"></svg>\n\n<script>a()</script>\n\ntext', push);
