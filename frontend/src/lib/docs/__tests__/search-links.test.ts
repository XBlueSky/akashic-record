...
describe("docsHitHref", () => {
	it("page with anchor (push corpus)", () =>
		expect(docsHitHref({ repo: "acme", path: "guide/setup.md", anchor: "quick-start" }, push)).toBe(
			"/docs/acme/latest/guide/setup#quick-start",
		));
	it("index page hit", () =>
	...
	it("pull corpus strips prefix", () =>
		expect(docsHitHref({ repo: "libx", path: "docs/guide/setup.md", anchor: "a" }, pull)).toBe(
			"/docs/libx/latest/guide/setup#a",
		));
