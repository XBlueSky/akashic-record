...
describe("resolveRelative", () => {
	it("sibling", () =>
		expect(resolveRelative("guide", "advanced.md")).toEqual({
			fullKey: "guide/advanced.md",
			escaped: false,
		}));
	it("climb", () =>
		expect(resolveRelative("guide", "../index.md")).toEqual({
			fullKey: "index.md",
			escaped: false,
		}));
	it("dot segment", () =>
		expect(resolveRelative("guide", "./sub/x.md")).toEqual({
			fullKey: "guide/sub/x.md",
			escaped: false,
		}));
	...
	it("pull prefix climb stays in corpus", () =>
		expect(resolveRelative("docs/guide", "../README.md")).toEqual({
			fullKey: "docs/README.md",
			escaped: false,
		}));
