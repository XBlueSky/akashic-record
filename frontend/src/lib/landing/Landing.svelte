<script lang="ts">
  ...
  import {
    ...
    computeEdges,
    edgesToConnections,
  } from "$lib/landing/constellation";
  ...
  <div class="absolute inset-0 flex items-center justify-center pointer-events-none z-[1]">
    {#if ready}
      ...
      >
        ...
        <p class="font-mono text-xs tracking-wide text-muted-foreground mb-9">{$t('landing.subtitle')}</p>

        {#if loading}
          <p class="text-muted-foreground font-mono text-xs tracking-wide">{$t('landing.scanning')}</p>
        {:else if repos.length === 0}
          <p class="text-muted-foreground font-mono text-xs tracking-wide">{$t('landing.noRepos')}</p>
        {:else}
          <div class="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-2.5">
            {#each repos as repo, i (repo.id)}
              ...
              >
                ...
                <span class="w-1.5 h-1.5 rounded-full bg-primary shrink-0 shadow-[0_0_8px_var(--color-primary)]"></span>
...
<style>
  ...
  .repo-bracket.left { margin-right: -4px; }
  .repo-bracket.right { margin-left: auto; }
  ...
  .repo-card:hover .w-1\.5 {
    box-shadow: 0 0 12px var(--color-primary), 0 0 24px rgba(59, 130, 246, 0.3);
  }
</style>
